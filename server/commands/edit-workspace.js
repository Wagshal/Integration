...
module.exports = class EditWorkspace {
    constructor(data) {
        ...
        this.workspaceConfig = JSON.parse(fs.readFileSync(WORKSPACES, 'utf8'));
    ...
    async addWorkspace() {
        ...
        if (this.isSucceeded.isSucceeded) {
            try {
                ...
                fs.writeFileSync(WORKSPACES, JSON.stringify(this.workspaceConfig), 'utf8');
        ...
        try {
            if (newUrl !== this.url && newName === this.name) {
                ...
                if (responseAdd.isSucceeded) {
                    this.name = saveName;
                    this.url = saveUrl;
                    let responeRemove = await this.removeWorkspace();
                    if (responeRemove.isSucceeded)
                        this.isSucceeded = { isSucceeded: true };
                    else {
                        this.name = newName;
                        this.url = newUrl;
                        responeRemove = await this.removeWorkspace();
